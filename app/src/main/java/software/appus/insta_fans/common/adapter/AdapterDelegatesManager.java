 ...
 * <li> {@link #onCreateViewHolder(ViewGroup, int, OnRecyclerItemClick)}: Must be called from {@link
 * RecyclerView.Adapter#onCreateViewHolder(ViewGroup, int)}</li>
 * <li> {@link #onBindViewHolder(Object, int, BaseViewHolder)}: Must be called from {@link
 * RecyclerView.Adapter#onBindViewHolder(RecyclerView.ViewHolder, int)} (BaseViewHolder, int)}</li>
 * </ul>
 * <p>
...
public class AdapterDelegatesManager<T> {
    ...
    /**
     * Used internally for {@link #(Object, int, BaseViewHolder)} as empty
     * payload parameter
     */
     ...
     */
    @NonNull
    public BaseViewHolder onCreateViewHolder(ViewGroup parent, int viewType, @Nullable OnRecyclerItemClick<T> listener) {
        AdapterDelegate<T> delegate = getDelegateForViewType(viewType);
        if (delegate == null) {
            throw new NullPointerException("No AdapterDelegate added for ViewType " + viewType);
        }

        BaseViewHolder vh = delegate.onCreateViewHolder(parent, listener);
        if (vh == null) {
    ...
    /**
     * Must be called from{@link RecyclerView.Adapter#onBindViewHolder(RecyclerView.ViewHolder, int)} (BaseViewHolder, int,
     * List)}
     *
     * @param item       Adapter's data source
    ...
    public void onBindViewHolder(@NonNull T item, int position,
                                 @NonNull BaseViewHolder<T> viewHolder, List payloads) {
     ...
     */
    public void onBindViewHolder(@NonNull T item, int position,
                                 @NonNull BaseViewHolder<T> viewHolder) {
        onBindViewHolder(item, position, viewHolder, PAYLOADS_EMPTY_LIST);
    }

    /**
     * Must be called from {@link RecyclerView.Adapter#onViewRecycled(RecyclerView.ViewHolder)} (BaseViewHolder)}
     *
    ...
    /**
     * Must be called from {@link RecyclerView.Adapter#onFailedToRecycleView(RecyclerView.ViewHolder)} (BaseViewHolder)}
     *
     ...
     */
    public boolean onFailedToRecycleView(@NonNull BaseViewHolder viewHolder) {
        AdapterDelegate<T> delegate = getDelegateForViewType(viewHolder.getItemViewType());
        if (delegate == null) {
            throw new NullPointerException("No delegate found for "
                    + viewHolder
    ...
    /**
     * Must be called from {@link RecyclerView.Adapter#onViewDetachedFromWindow(RecyclerView.ViewHolder)} (BaseViewHolder)}
     *
     ...
     * @param viewHolder Holder of the view being attached
     */
    public void onViewDetachedFromWindow(BaseViewHolder viewHolder) {
        AdapterDelegate<T> delegate = getDelegateForViewType(viewHolder.getItemViewType());
        if (delegate == null) {
            throw new NullPointerException("No delegate found for "
                    + viewHolder
